...
def median_filter_1d(x: Tensor, kernel_size: int) -> Tensor:
    ...
    -------
    filtered : Tensor
        Filtered tensor of shape ``(T,)``.
    """
    assert kernel_size % 2 == 1  # sanity-check
    pad = kernel_size // 2

    # unfold requires 2D, add batch dimension: (T,) -> (1, T)
    x_padded = torch.nn.functional.pad(
        x.unsqueeze(0), (pad, pad), mode="constant", value=0
    )

    # sliding windows: (1, T, kernel_size) -> median -> (1, T) -> squeeze -> (T,)
    return (
        x_padded.unfold(dimension=1, size=kernel_size, step=1)
        .median(dim=-1)
        .values.squeeze(0)
    )
